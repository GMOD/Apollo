...
class RoleService {
    ...
    def initRoles() {
        if (Role.count == 0) {
            def userRole = new Role(name: GlobalPermissionEnum.USER.name(), rank: GlobalPermissionEnum.USER.rank).save(failOnError: true, flush: true)
            ...
            def instructorRole = new Role(name: GlobalPermissionEnum.INSTRUCTOR.name(), rank: GlobalPermissionEnum.INSTRUCTOR.rank).save(failOnError: true, flush: true)
            ...
            def adminRole = new Role(name: GlobalPermissionEnum.ADMIN.name(), rank: GlobalPermissionEnum.ADMIN.rank).save(failOnError: true, flush: true)
