...
class BamService {
    ...
    @NotTransactional
    def processSequence(JSONArray featuresArray, String sequenceName, BAMFileReader bamFileReader, int start, int end) {
        Collection<SAMEntry> samEntriesStart = bamFileReader.query(sequenceName,start,end)
//        Collection<SAMEntry> samEntriesStop = bamFileReader.query(sequenceName,start,end)

        Integer chrStart = samEntriesStart.first().start
        Integer chrStop = samEntriesStart.last().stop
//        Integer chrStop = samEntriesStart.last().stop
//        Integer chrStop = bamFileReader.getChrStop(sequenceName)

        double max = bamFileReader.max()
        double min = bamFileReader.min()
//        double mean = bamFileReader.mean()
        ...
        edu.unc.genomics.Contig contig = bamFileReader.query(interval)
    ...
    @NotTransactional
    def processProjection(JSONArray featuresArray, MultiSequenceProjection projection, BAMFileReader bamFileReader , int start, int end) {
