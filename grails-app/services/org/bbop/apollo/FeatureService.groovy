...
//        }
    ...
    log.debug "found a top level gene ${gene}"
    if (gene) {
          ...
          if (!gene && feature instanceof Gene && !(feature instanceof Pseudogene)) {
            ...
            if (tmpTranscript && tmpGene && overlapperService.overlaps(tmpTranscript, tmpGene)) {
              ...
              if (jsonTranscript.has(FeatureStringEnum.PARENT.value)) {
                log.debug "I guess we had a gene and a parent . . this is confusing how we get here "
    ...
    log.debug "did we generate the gene? ${gene}"
    if (gene == null) {
      log.debug "Gene was null, so not sure if we are generating ${gene}"
 ...
 */
  ...
  @Transactional
  Feature createDbXrefs(String inputString, Feature feature) {
    def (dbString, accessionString) = inputString.split(":")
    DB newDB = DB.findOrSaveByName(dbString)
    DBXref newDBXref = DBXref.findOrSaveByDbAndAccession(newDB, accessionString).save()
    feature.addToFeatureDBXrefs(newDBXref)
    feature.save()
  }

  @Transactional
  Feature createNote(String noteString, Feature feature, Integer rank) {
  ...
  @Transactional
  def createSynonyms(String synonymString, Feature feature) {
  ...
  Feature convertJSONToFeature(JSONObject jsonFeature, Sequence sequence) {
    Feature gsolFeature
    log.debug "input json feature ${jsonFeature as JSON} for ${sequence}"
    try {
      ...
      if (jsonFeature.has(FeatureStringEnum.EXPORT_ALIAS.value.toLowerCase())) {
        def aliasObject = jsonFeature.get(FeatureStringEnum.EXPORT_ALIAS.value.toLowerCase())
        if (aliasObject instanceof JSONArray) {
          for (String synonymString in (aliasObject as JSONArray)) {
            createSynonyms(synonymString, gsolFeature)
          }
        } else {
          createSynonyms(aliasObject as String, gsolFeature)
        }
      }
      ...
      if (jsonFeature.has(FeatureStringEnum.EXPORT_NOTE.value.toLowerCase())) {
        def noteObject = jsonFeature.get(FeatureStringEnum.EXPORT_NOTE.value.toLowerCase())
        int rank = 0
        if (noteObject instanceof JSONArray) {
          for (String noteString in (noteObject as JSONArray)) {
            createNote(noteString, gsolFeature, rank++)
          }
        } else {
          createNote(noteObject as String, gsolFeature, rank++)
        }
      }
...
//                String propertyValue = property.get(FeatureStringEnum.VALUE.value)
  ...
  void updateGeneBoundaries(Gene gene) {
    log.debug "updateGeneBoundaries ${gene}"
    if (gene == null) {
      return;
    }
    ...
    for (Transcript t : transcriptService.getTranscripts(gene)) {
      log.debug "transscript ${t} "
      log.debug "fmin / fmax ${t.fmin} / ${t.fmax} "
    ...
    log.debug "feature locations ${gene.featureLocations}"
    log.debug "single feature location ${gene.featureLocation}"
