...
class RequestHandlingService {
    ...
    private JSONObject wrapFeature(JSONObject jsonObject, Feature feature) {
        ...
        if (feature instanceof Gene) {
            for (childFeature in feature.parentFeatureRelationships.childFeature) {
    ...
    JSONObject setTranslationStart(JSONObject inputObject) throws AnnotationException {
        ...
        if (transcriptsToUpdate.size() > 0) {
            JSONObject updateFeatureContainer = jsonWebUtilityService.createJSONFeatureContainer()
            for (t in transcriptsToUpdate) {
                updateFeatureContainer.getJSONArray(FeatureStringEnum.FEATURES.value).put(featureService.convertFeatureToJSON(t))
            }
            if (sequence) {
                AnnotationEvent annotationEvent = new AnnotationEvent(
                        features: updateFeatureContainer,
    ...
    def setAcceptor(JSONObject inputObject, boolean upstreamDonor) {
        ...
        for (int i = 0; i < features.length(); ++i) {
            ...
            if (transcriptsToUpdate.size() > 0) {
                JSONObject updateFeatureContainer = jsonWebUtilityService.createJSONFeatureContainer()
                for (def updatedTranscript in transcriptsToUpdate) {
                    updateFeatureContainer.getJSONArray(FeatureStringEnum.FEATURES.value).put(featureService.convertFeatureToJSON(updatedTranscript))
                }
                if (sequence) {
                    AnnotationEvent annotationEvent = new AnnotationEvent(
                            features: updateFeatureContainer,
...
//                    }
    ...
    def splitTranscript(JSONObject inputObject) {
        ...
        for (owner in transcript1.owners) { transcript2.addToOwners(owner) }
    ...
    def mergeTranscripts(JSONObject inputObject) {
        ...
        List<Transcript> sortedTranscripts = [transcript1, transcript2]
        sortedTranscripts.sort(Comparator.comparingInt { Transcript t -> t.fmin }
            .thenComparingInt { Transcript t -> t.fmax }
    ...
    def dissociateFeatureFromGene(JSONObject inputObject) {
        ...
        for (String key in jsonObject.keySet()) {
