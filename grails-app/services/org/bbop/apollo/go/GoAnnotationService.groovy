...
//                }
  ...
  def deleteAnnotations(JSONArray featuresArray) {
    ...
    for (Feature thisFeature in features) {
      ...
      List<GoAnnotation> annotations = []
      if (parentFeature) {
        annotations.addAll(GoAnnotation.executeQuery("select ga from GoAnnotation ga join ga.feature f where f = :parentFeature ", [parentFeature: parentFeature]))
      }
