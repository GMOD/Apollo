...
String usageString = "migrate_users1to2.groovy <options>" +
        "Example: \n" +
        "./migrate_users1to2.groovy -username1 demo -password1 demo -databaseurl databaseurl -username2 ndunn@me.com -password2 demo  -organism Honey2 -destinationurl http://localhost:8080/WebApollo2/ "
...
try {
    ...
    if (!(options?.destinationurl && options?.databaseurl && options?.username2 && options?.password2 && options?.username1 && options?.password1)) {
        println "\n"+usageString
        return
    }
} catch (e) {
