...
my $feature_result=`curl -b cookies.txt -c cookies.txt --data '{ "track": "$track", "features": $feature_data, "operation": "add_transcript" }' $url/AnnotationEditorService 2> /dev/null`;
